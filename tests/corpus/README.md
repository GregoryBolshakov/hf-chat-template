...
<model-slug>/
  ...
  tokenizer_config.json  trimmed config (chat_template + special tokens) — the Rust render source
...
The Rust side renders each case via the public `ChatTemplate::builder_from_config` path and
asserts equality with `expected/<name>.txt`. CI needs **no Python** — only regenerating the
references does.
...
`tools_weather` / `tool_use` cases exercise `tojson` key-order and (for Hermes) the named
`tool_use` sub-template with Jinja macros. Growing toward ~50 models and adding date-pinned
(`strftime_now`) models is tracked for M5.

...
- Loading a template from a raw string or a parsed `tokenizer_config.json`.
- Special-token substitution into the render context (section 8).

Optional, feature-gated (off by default):

- `hub`: fetch `tokenizer_config.json` from the Hugging Face Hub (`from_hub`).
...
- `ChatTemplate::builder(&str)` / `builder_from_config(&TokenizerConfig)` — `ChatTemplateBuilder`
  for non-default options: `template_name`, `clock`, `pycompat`, `undefined_behavior`, `build`.
...
tokenizer_config.json  # trimmed: chat_template + special tokens
